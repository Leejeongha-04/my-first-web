...
- [x] [Ch11] Supabase CLI를 이용한 RLS 마이그레이션 생성
- [x] [Ch11] `profiles` 테이블 RLS 정책 적용 (조회:전체, 수정:본인)
- [x] [Ch11] `posts` 테이블 RLS 정책 적용 (조회:전체, 생성/수정/삭제:본인)
- [x] [Ch11] RLS 적용에 따른 기능 동작 테스트 (비로그인/A/B 시나리오)
- [x] [Ch11] 프로젝트 내 민감 키(service_role) 노출 여부 점검 (grep)
- [x] [Ch11] 빌드 및 배포 검증 (`npm run build`)
